 ...
 */
    ...
    func selectCategories(index:Int, selected:Bool)
}
 ...
 */
    ...
    open class func initTagView2(_ contents: [String]?) -> TagView {
        let bun = Bundle(for: self.classForCoder())
        let tagView = bun.loadNibNamed("TagView", owner: self, options: nil)?.last as? TagView
        if tagView != nil {
            if contents != nil && (contents?.count)! > 0 {
 ...
 */
    ...
    open func setupInitialConstraintWRTView(_ view: UIView) {
        ...
        topConst = NSLayoutConstraint.init(item: self, attribute: .top, relatedBy: .equal, toItem: self.mainView, attribute: .top, multiplier: 1.0, constant: 0.0)
    ...
    func addNewTag(_ newTagString: String) {
        if (newTagString.count) > 0 {
            ...
            self.collectionView?.scrollToItem(at: lastItemIndex, at: UICollectionView.ScrollPosition.top, animated: false)
...
//        if (statusText?.count)! < 0 || statusText == "" {
...
extension TagView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, TagViewCellDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        self.autoWidth += Int(self.sizingCell!.systemLayoutSizeFitting(UIView.layoutFittingCompressedSize).width)
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if self.delegate != nil {
            self.delegate?.selectCategories(index: indexPath.row, selected:tags[indexPath.row].selected)
        }
        self.collectionView.reloadData()
    }
    ...
    func configureCell(_ cell: TagViewCell, forIndexPath indexPath: IndexPath) {
        ...
        if !self.isChecked!{
            cell.backgroundColor = tag.selected ? Constants.RGB_COLOR : UIColor(red: 0.8, green: 0.8, blue: 0.8, alpha: 1)
            cell.tagName.textColor = tag.selected ? UIColor.white : UIColor(red: 0.1, green: 0.1, blue: 0.1, alpha: 1)
        }else{
            cell.backgroundColor = Constants.RGB_COLOR
