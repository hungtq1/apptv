...
public enum PopupCustomOption {
    case layout(FLPopupViewController.PopupLayout)
    case animation(FLPopupViewController.PopupAnimation)
    case backgroundStyle(FLPopupViewController.PopupBackgroundStyle)
...
open class FLPopupViewController: UIViewController {
    ...
    fileprivate var layout: PopupLayout = .top
    ...
    fileprivate var closedHandler: ((FLPopupViewController) -> Void)?
    fileprivate var showedHandler: ((FLPopupViewController) -> Void)?
...
// MARK: - Publics
public extension FLPopupViewController {

    // MARK: Classes
    class func create(_ parentViewController: UIViewController) -> FLPopupViewController {
        let controller = FLPopupViewController()
    ...
    func customize(_ options: [PopupCustomOption]) -> FLPopupViewController {
        customOptions(options)
        return self
    }

    @discardableResult
    func show(_ childViewController: UIViewController,_ layout: PopupLayout = .center) -> FLPopupViewController {
        ...
        self.layout = layout
        show(self.layout, animation: animation) {
    ...
    func didShowHandler(_ handler: @escaping (FLPopupViewController) -> Void) -> FLPopupViewController {
        self.showedHandler = handler
        return self
    }

    func didCloseHandler(_ handler: @escaping (FLPopupViewController) -> Void) -> FLPopupViewController {
...
// MARK: Privates
private extension FLPopupViewController {
    ...
    func registerNotification() {
        NotificationCenter.default.addObserver(self, selector: #selector(FLPopupViewController.popupControllerWillShowKeyboard(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(FLPopupViewController.popupControllerWillHideKeyboard(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(FLPopupViewController.popupControllerDidHideKeyboard(_:)), name: UIResponder.keyboardDidHideNotification, object: nil)
    }
    ...
    func registerTapGesture() {
        let gestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(FLPopupViewController.didTapGesture(_:)))
    ...
    @objc func popupControllerWillHideKeyboard(_ : Notification) {
        back()
    }

    @objc func popupControllerDidHideKeyboard(_ : Notification) {
        self.isShowingKeyboard = false
    }

    // Tap Gesture
    @objc func didTapGesture(_ : UITapGestureRecognizer) {
        self.closePopup {
            print("closed popup")
        }
    }
...
extension FLPopupViewController: UIScrollViewDelegate {
...
extension FLPopupViewController: UIGestureRecognizerDelegate {
